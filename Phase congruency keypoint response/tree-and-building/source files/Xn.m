image = im2uint8(imread('images/Xn.png'));
...
%         figure;
%         
%         imagesc(normalized_eo{i, j});
%         axis off; % 关闭坐标轴显示
%         colormap gray;
%         
%         title(['Image (' num2str(i) ', ' num2str(j) ')']);
        ...
        % 创建文件名
        filename = sprintf('E:/code/LGHD-master/tree-and-building/result/Xn/normalized_image_%d_%d.png', i, j);
...
% figure; % 创建一个新的图形窗口
% imagesc(onePercentage); % 显示onePercentage矩阵
% axis off; % 关闭坐标轴显示
% colormap jet;
% colorbar; % 显示颜色条，以便知道不同颜色代表的数值
% title('Proportion of Ones in Each Matrix'); % 设置图像标题
...
savePath = 'E:/code/LGHD-master/tree-and-building/result/Xn/onePercentage_Image.png'
% 保存RGB图像
imwrite(onePercentage_Image, savePath);



% onePercentage_Image = uint8(onePercentage * 255);
% 
% savePath = 'E:/LGHD-master/tree-and-building/result/Xn/onePercentageImage.png'
% % 保存RGB图像
% imwrite(onePercentage_Image, savePath);
