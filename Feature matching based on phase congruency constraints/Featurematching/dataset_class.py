...
class PointDataset(Dataset):
    ...
    def _solveSingleImage(self, image_ori_base):
        # 随机翻转
    ...
    def __getitem__(self, item):
        ...
        return self.solveSingleImage(image_ori_base)
...
class PairPointDataset(PointDataset):
    def __init__(self,root_dir: str,
                 random_num: int, image_row: int, image_col: int):
        super().__init__(root_dir, random_num, image_row, image_col)
        self.solvedImages = []
        for idx, img in enumerate(self.image_names):
            img:str
            if img.count("_X") > 0:
                continue
            self.solvedImages.append(ImagePair(img))

    def __len__(self):
        return len(self.solvedImages)

    def __getitem__(self, item):
        imgpair:ImagePair = self.solvedImages[item]
        currentImages = imgpair.readImagePair()
        solvedImages = []
        for img in currentImages:
            solvedImages.append(self._solveSingleImage(img))
        return [solvedImages]
...
class ImagePair():
    def __init__(self, ori:str):
        ...
        suffix = ".jpg"
        name = ""
        if ori.endswith(suffix):
            name = ori[0:len(ori) - len(suffix)]
        self.X = name + "_X" + suffix

    def readImagePair(self):
        ori_img = cv2.imread(self.originPath)
        ori_img = cv2.cvtColor(ori_img, cv2.COLOR_BGR2RGB)

        X_img = cv2.imread(self.X)
        X_img = cv2.cvtColor(X_img, cv2.COLOR_BGR2RGB)

        return [ori_img, X_img]