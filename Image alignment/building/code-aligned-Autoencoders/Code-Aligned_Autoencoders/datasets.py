...
def _mydata(clip=True):
    """Load mydata dataset from .mat"""
    mat = loadmat("data/mydata/mydata.mat")
    print("here")
    t1 = np.array(mat["t1"], dtype=np.single)
    t2 = np.array(mat["t2"], dtype=np.single)
    if clip:
    ...
    savemat(
        "data/mydata/mydata_clipped.mat",
        {"t1": t1.numpy(), "t2": t2.numpy(), "ROI": change_mask.numpy()},
    )
    return t1, t2, change_mask

def _mydata2(clip=True):
    """Load mydata dataset from .mat"""
    mat = loadmat("data/mydata/mydata2.mat")
    print("here")
    t1 = np.array(mat["t1"], dtype=np.single)
    t2 = np.array(mat["t2"], dtype=np.single)
    if clip:
    ...
    savemat(
        "data/mydata/mydata2_clipped.mat",
        {"t1": t1.numpy(), "t2": t2.numpy(), "ROI": change_mask.numpy()},
    )
    return t1, t2, change_mask

def _tree_and_building(clip=True):
    """Load mydata dataset from .mat"""
    mat = loadmat("data/mydata/mydata_tree-and-building.mat")
    print("here")
    t1 = np.array(mat["t1"], dtype=np.single)
    t2 = np.array(mat["t2"], dtype=np.single)
    if clip:
    ...
    savemat(
        "data/mydata/mydata_tree-and-building_clipped.mat",
        {"t1": t1.numpy(), "t2": t2.numpy(), "ROI": change_mask.numpy()},
    )
    return t1, t2, change_mask

def _building(clip=True):
    """Load mydata dataset from .mat"""
    mat = loadmat("data/mydata/mydata_building.mat")
    print("here")
    t1 = np.array(mat["t1"], dtype=np.single)
    t2 = np.array(mat["t2"], dtype=np.single)
    if clip:
    ...
    savemat(
        "data/mydata/mydata_building_clipped.mat",
        {"t1": t1.numpy(), "t2": t2.numpy(), "ROI": change_mask.numpy()},
    )
    return t1, t2, change_mask

def _playground(clip=True):
    """Load mydata dataset from .mat"""
    mat = loadmat("data/mydata/mydata_playground.mat")
    print("here")
    t1 = np.array(mat["t1"], dtype=np.single)
    t2 = np.array(mat["t2"], dtype=np.single)
    if clip:
    ...
    savemat(
        "data/mydata/mydata_playground_clipped.mat",
        {"t1": t1.numpy(), "t2": t2.numpy(), "ROI": change_mask.numpy()},
    )
    return t1, t2, change_mask
...
DATASETS = {
    ...
    "mydata": _mydata,
    "mydata2": _mydata2,
    "tree-and-building": _tree_and_building,
    "building": _building,
...
def fetch(name, **kwargs):
    ...
    if not tf.config.list_physical_devices("GPU"):
        dataset = [
            tf.image.central_crop(tensor, 1) for tensor in [x_im, y_im, target_cm]  #0.1
