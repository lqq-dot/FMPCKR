...
from decorators import image_to_tensorboard,write_image_to_png
   ...
   # @tf.function
    def __call__(self, inputs, training=False):
        ...
        if training:
            ...
            x_tilde =  tf.image.rgb_to_grayscale(x_tilde)
            y_tilde =  tf.image.rgb_to_grayscale(y_tilde)
            x_hat, y_hat = (
                self.dec_x(y_code, name="x_hat"),
                self.dec_y(x_code, name="y_hat"),
            )
            x_hat =  tf.image.rgb_to_grayscale(x_hat)
            y_hat =  tf.image.rgb_to_grayscale(y_hat)


            lambda1 = 0.5
            lambda2 = 0.2
            Xn = lambda1 * x_code + lambda2 * x_tilde + (1 - lambda1 - lambda2) * y_hat
            Yn = lambda1 * y_code + lambda2 * y_tilde + (1 - lambda1 - lambda2) * x_hat
            Xn_filename = tf.constant("E:/data-alignment/tree-and-building/tree-and-building-result")+ tf.constant(f"/Xn.png")
            Yn_filename = tf.constant("E:/data-alignment/tree-and-building/tree-and-building-result")+ tf.constant(f"/Yn.png")

            write_image_to_png(Xn, Xn_filename)
            write_image_to_png(Yn, Yn_filename)
...
def test(DATASET="Texas", CONFIG=None):
    ...
    # EVALUATE2 = datasets.fetch('mydata2', **CONFIG)
